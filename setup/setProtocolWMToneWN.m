...
soundParams.isi = 500;
CNMStimFinal =audWM(interTrialLuminance,soundParams,maxWidth,maxHeight,scaleFactor,interTrialLuminance);
...
ts6 = trainingStep(lpTM  , CNMStimHard, performanceCriterion(.80, int8(200)), noTimeOff(), svnRev,svnCheckMode);

ts7 = trainingStep(lpTM  , CNMStimFinal, repeatIndefinitely(), noTimeOff(), svnRev,svnCheckMode);

%p=protocol('mouse CNM task',{ts1, ts2, ts3, ts4, ts5 ts6}); %normal setup
p=protocol('mouse WM task WN vs tone',{ts1, ts2, ts3, ts4, ts5, ts6, ts7}); %to test CNM/goNoGo
